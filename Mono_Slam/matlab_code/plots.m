...
%-----------------------------------------------------------------------
%%      
% Modify:Zhonghao Zhang
% 
% 使用双目相机，改为plots_1，plots_2,内核不变
%% 
...
predicted_index = 0;
predicted_measurements = get_predicted_measurements(filter_r);
S_predicted = get_S_predicted(filter_r);          

for i=1:length(features_info_r)

    if (~isempty(features_info_r(i).h)&&~isempty(features_info_r(i).S))

        imagesc( features_info_r(i).h(1) - features_info_r(i).half_patch_size_when_matching,...
            features_info_r(i).h(2) - features_info_r(i).half_patch_size_when_matching, features_info_r(i).patch_when_matching);

        if features_info_r(i).low_innovation_inlier
            plotUncertainEllip2D( features_info_r(i).S,...
                features_info_r(i).h, chi_095_2, 'r', 4 )
            plot( features_info_r(i).h(1), features_info_r(i).h(2),'r+','Markersize',10);
        end
        if features_info_r(i).high_innovation_inlier
            plotUncertainEllip2D( features_info_r(i).S,...
                features_info_r(i).h, chi_095_2, 'r', 2 )
            plot( features_info_r(i).h(1), features_info_r(i).h(2),'r+','Markersize',10);
        end
        if (features_info_r(i).individually_compatible)&&(features_info_r(i).high_innovation_inlier==0)&&(features_info_r(i).low_innovation_inlier==0)
            plotUncertainEllip2D( features_info_r(i).S,...
                features_info_r(i).h, chi_095_2, 'm', 2 )
            plot( features_info_r(i).h(1), features_info_r(i).h(2),'m+','Markersize',10);
        end
        
        if ~features_info_r(i).individually_compatible
            plotUncertainEllip2D( features_info_r(i).S,...
                features_info_r(i).h, chi_095_2, 'b', 2 )
            plot( features_info_r(i).h(1), features_info_r(i).h(2),'b+','Markersize',10);
        end
        
        if features_info_r(i).individually_compatible
            plot( features_info_r(i).z(1), features_info_r(i).z(2),'g+','Markersize',10);
        end
...
x_k_k = get_x_k_k(filter_r);
p_k_k = get_p_k_k(filter_r);
...
for i=1:length(features_info_r)
    ...
    if strcmp(features_info_r(i).type, 'cartesian')
    ...
    if strcmp(features_info_r(i).type, 'inversedepth')
